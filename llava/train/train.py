...
def preprocess_v1(
    ...
    print("uuuuu", conversations)
    print("uuuuuu", input_ids, targets)
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version.startswith("v1"):
        ...
        #print("******************")
        #preprocess_v1([[{'from': 'human', 'value': '<image>\nWhat kind of animal is this?'}, {'from': 'gpt', 'value': 'dog'}, {'from': 'human', 'value': 'Is it big?'}, {'from': 'gpt', 'value': 'no. its little'}]], tokenizer, has_image=has_image)
        #print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
        return preprocess_v1(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "mpt":
        return preprocess_mpt(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "llama_v3":#"llama3":
        #print("Preproc llama3")

        return preprocess_llama3(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    #print("Preproc common")
